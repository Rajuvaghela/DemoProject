...
public class Utility {
    ...
    static Typeface tf;
    private static Context context = Infranet.context;
    private static SharedPreferences preferences;
    public static Context act = Infranet.getAppContext();
    ...
    public static void showSuccessAlertDialog(Activity ac, String s) {
        ColorDialog colorDialog = new ColorDialog(ac);
        colorDialog.setContentText(s);
        colorDialog.setCancelable(false);
        colorDialog.setAnimationEnable(true);
        colorDialog.setPositiveListener(ac.getString(R.string.ok), new ColorDialog.OnPositiveListener() {
            @Override
        ...
        for (int j = 0; j < tabsCount; j++) {
            ...
            for (int i = 0; i < tabChildsCount; i++) {
                ...
                if (tabViewChild instanceof TextView) {
                    ((TextView) tabViewChild).setTypeface(Utility.getTypeFaceTab());
                }
            }
        }
    }
    ...
    //region Show Dialog
    public static void showDialog(final Activity ac, String s) {
        ColorDialog colorDialog = new ColorDialog(ac);
        colorDialog.setContentText(s);
        colorDialog.setCancelable(false);
        colorDialog.setAnimationEnable(true);
        colorDialog.setPositiveListener(act.getString(R.string.ok), new ColorDialog.OnPositiveListener() {
            @Override
            public void onClick(ColorDialog dialog) {
                dialog.dismiss();
            }
    ...
    public static void showAlertDialog(final Activity act, String s) {
        new PromptDialog(act)
                .setDialogType(PromptDialog.DIALOG_TYPE_SUCCESS)
                .setAnimationEnable(true)
                .setContentText(s)
                .setNegativeListener(act.getString(R.string.negativeButton), new PromptDialog.OnNegativeListener() {
                    @Override
                    public void onClick(PromptDialog dialog) {
                        dialog.dismiss();
                    }
                })
                .setPositiveListener(act.getString(R.string.positiveButton), new PromptDialog.OnPositiveListener() {
                    @Override
                    public void onClick(PromptDialog dialog) {
                        ((MainActivity) act).onLogOut();
                    }
                }).show();
    }
