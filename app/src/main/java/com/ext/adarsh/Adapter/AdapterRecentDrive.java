...
public class AdapterRecentDrive extends RecyclerView.Adapter<AdapterRecentDrive.MyViewHolder> implements Filterable {
    ...
    public AdapterRecentDrive(List<BeanFiles> messqageList, Activity activity) {
    ...
    private void filesharingDialog(int pos, final String fileid, final String oldFileName,
        ...
        ll_move_dialog.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                RecentFileFragments.rl_move_or_copy.setVisibility(View.VISIBLE);
                RecentFileFragments.upload_float.setVisibility(View.GONE);
                            ...
                            if (array.length() != 0) {
                                for (int i = 0; i < array.length(); i++) {
                                    if (array.optJSONObject(i).getString("Status").equalsIgnoreCase("Success")) {
                                        ...
                                        pd.dismiss();
                                        fileshare.dismiss();
                                        RecentFileFragments.getFileData();
                                    } else {
                                        pd.dismiss();
                                        Toast.makeText(activity, "" + array.optJSONObject(i).getString("Status_Message"), Toast.LENGTH_SHORT).show();
                                    }
