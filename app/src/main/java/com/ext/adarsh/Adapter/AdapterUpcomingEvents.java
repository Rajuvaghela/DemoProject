...
public class AdapterUpcomingEvents extends RecyclerView.Adapter<AdapterUpcomingEvents.MyViewHolder> {
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        TextView btn_going;
        @BindView(R.id.btn_notgoing)
        TextView btn_notgoing;

        @BindView(R.id.txt_private)
        TextView txt_private;

        @BindView(R.id.ll_not_going_in_event)
        LinearLayout ll_not_going_in_event;

        @BindView(R.id.ll_going_in_event)
        LinearLayout ll_going_in_event;

        @BindView(R.id.ll_event_main)
        LinearLayout ll_event_main;

        @BindView(R.id.imageViewEvent)
        ImageView imageViewEvent;

        @BindView(R.id.ln_goingnot)
        LinearLayout ln_goingnot;
    ...
    public AdapterUpcomingEvents(Activity activity, ArrayList<BeanEvents> beanEventsArrayList) {
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        holder.btn_going.setTypeface(Utility.getTypeFaceTab());
        holder.btn_notgoing.setTypeface(Utility.getTypeFaceTab());
        ...
        if (!beanEventsArrayList.get(position).goingFlag.equalsIgnoreCase("Y")) {
            holder.ln_goingnot.setVisibility(View.GONE);
        }
        ...
        holder.ll_going_in_event.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                eventgoing(beanEventsArrayList.get(position).eventId);
            }
        });

        holder.ll_not_going_in_event.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                eventnotgoing(beanEventsArrayList.get(position).eventId);
            }
        });

        holder.ll_event_main.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    public void eventgoing(final String eid) {
        pd.show();
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.Upcoming_Event_Going, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Log.e(AppConstant.TAG, response);
                    try {
                        JSONObject object = new JSONObject(response);
                        if (object.has("Upcoming_Event_Going")) {
                            JSONArray jsonArray = object.getJSONArray("Upcoming_Event_Going");
    ...
    public void eventnotgoing(final String eid) {
        pd.show();
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.Upcoming_Event_NotGoing, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Log.e(AppConstant.TAG, response);
                    try {
                        JSONObject object = new JSONObject(response);
                        if (object.has("Upcoming_Event_NotGoing")) {
                            JSONArray jsonArray = object.getJSONArray("Upcoming_Event_NotGoing");
                            if (jsonArray.length() != 0) {
                                for (int i = 0; i < jsonArray.length(); i++) {
                                    if (jsonArray.optJSONObject(i).getString("Status").equalsIgnoreCase("Success")) {
                                        Toast.makeText(activity, "" + jsonArray.optJSONObject(i).getString("Status_Message"), Toast.LENGTH_SHORT).show();
    ...
    private void showAlertDialog3(int position) {
        ...
        d_time.setText(beanEventDetails.get(0).startTime);
