...
public class AdapterPhotoComment extends RecyclerView.Adapter<AdapterPhotoComment.MyViewHolder> {

    private List<BeanPhotoComment> sub_comment_list;
    Activity activity;
    Dialog dd;
    ...
    ProgressDialog pd;
    AdapterPhotoComentReply sub_comment_adapter;
    public static String manage;
    String albumDetailId;
    ...
    public AdapterPhotoComment(List<BeanPhotoComment> knowledgeList, Activity contex, String manage, String albumDetailId) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        final BeanPhotoComment feed_comment = sub_comment_list.get(position);
        ...
        holder.tv_coment_person.setText(feed_comment.commentPeopleFullName);
        Glide.with(activity).load(feed_comment.commentPeopleProfileImage).into(holder.iv_coment_person);
        holder.tv_comment.setText(feed_comment.comment);
        holder.tv_no_of_like.setText(feed_comment.likeCount);

        if (feed_comment.commentLikeDislikeFlag.equals("A")) {
        ...
        holder.tv_comment_reply.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                CommentReply(position, feed_comment.likeCount,
                        feed_comment.postCommentId, feed_comment.postId, feed_comment.postPeopleId);
            }
        });
    }


    void CommentReply(final int position, String like_count, final String postCommentId,
                                      final String postId, final String commentPepoleI) {
        ...
        ll_reply_comment.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                FeedNewsCommentReply(et_sub_comment.getText().toString(), postCommentId, albumDetailId, postId, commentPepoleI, position);
            }
        });
        ...
        if (sub_comment_list.get(position).photo_Comment_Replay_Array != null){
            sub_comment_adapter = new AdapterPhotoComentReply(sub_comment_list.get(position).photo_Comment_Replay_Array, activity);
            rv_sub_comment.setAdapter(sub_comment_adapter);
        }else {
            rv_sub_comment.setAdapter(null);
        }
    ...
    void FeedNewsCommentReply(final String comment_reply, final String postCommentId,
                              ...
                              final String commentPepoleId, final int position) {
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.Photo_Comment_Replay, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Log.e("sub_pos_res", response);
                    try {
                        JSONObject object = new JSONObject(response);
                        if (object.has("Photo_Comment_Replay")) {
                            JSONArray array = object.getJSONArray("Photo_Comment_Replay");
                            if (array.length() != 0) {
                                for (int i = 0; i < array.length(); i++) {
                                    if (array.optJSONObject(i).getString("Status").equalsIgnoreCase("Success")) {
                        ...
                        if (object.has("Photo_Comment_Replay_Array")) {
                            JSONArray array = object.getJSONArray("Photo_Comment_Replay_Array");
                            if (array.length() != 0) {
                                for (int i = 0; i < array.length(); i++) {
                                    ...
                                    /*sub_comment_list.get(position).photo_Comment_Replay_Array.add(new BeanPhotoSubComment());
                                    sub_comment_adapter.notifyDataSetChanged();*/

                                    sub_comment_list.get(position).photo_Comment_Replay_Array.add(new BeanPhotoSubComment(FullName,ProfileImage,PostProfileImage,PostCommentId,AlbumId,AlbumDetailId,PostId,PostPeopleId,CommentPeopleId,ParentId,Comment,LikesUserId,LikeCount,LikeStatus));
                                    sub_comment_adapter.notifyDataSetChanged();
                                    sub_comment_adapter.notifyItemInserted(sub_comment_list.size());
                                    rv_sub_comment.scrollToPosition(sub_comment_list.size());
                ...
                protected Map<String, String> getParams() throws AuthFailureError {
                    ...
                    map.put("PostPeopleId", commentPepoleId);
                    map.put("Comment", comment_reply);
                    map.put("LoginId", Utility.getPeopleIdPreference());

                    Log.e("PostCommentId",""+ postCommentId);
                    Log.e("AlbumDetailId",""+ albumDetailId);
                    Log.e("PostId",""+ postId);
                    Log.e("PostPeopleId",""+ commentPepoleId);
                    Log.e("Comment",""+ comment_reply);
