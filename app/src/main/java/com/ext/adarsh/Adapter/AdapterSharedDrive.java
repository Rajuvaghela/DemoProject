...
import com.ext.adarsh.Activity.SubFolderActivityShared;
...
import com.ext.adarsh.Bean.BeanShareDrive;
...
public class AdapterSharedDrive extends RecyclerView.Adapter<AdapterSharedDrive.MyViewHolder> implements Filterable {

    private List<BeanShareDrive> filesList;
    private List<BeanShareDrive> array;
    ...
    public static Adapter1FileShareed3 adapter1people;
    public static RecyclerView.LayoutManager recylerViewLayoutManager3;
    ...
    ArrayList<ModelMutilpleDelete> checklistfile = new ArrayList<>();


    public class MyViewHolder extends RecyclerView.ViewHolder {
        public TextView title;
        LinearLayout ll_file, profile_image, profile_image2, lnmain2;
        ...
        public MyViewHolder(View view) {
            ...
            folderimage = (ImageView) view.findViewById(R.id.folderimage);
            firstname2 = (TextView) view.findViewById(R.id.firstname2);
            profile_image = (LinearLayout) view.findViewById(R.id.profile_image);
            profile_image2 = (LinearLayout) view.findViewById(R.id.profile_image2);
            lnmain2 = (LinearLayout) view.findViewById(R.id.lnmain2);

            // message = (TextView) view.findViewById(R.id.file_message);
        }
    }

    public AdapterSharedDrive(List<BeanShareDrive> messqageList, Activity activity) {
        this.filesList = messqageList;
        this.array = messqageList;
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        final BeanShareDrive movie = filesList.get(position);
        ...
        holder.lnshareopen.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //fileid, oldFileName, isFileFlag
    /*            AppConstant.MoveFileId = filesList.get(0).fileId;
                AppConstant.MoveFileName = filesList.get(0).folderName;
                AppConstant.MoveFilePath = filesList.get(0).fullPath;
                AppConstant.MoveIsFileFlag = filesList.get(0).isFile;*/
                filesharingDialog(position, movie.fileId, movie.folderName, movie.isFile, movie.filePath, movie.icon, movie.privecyFlag);
            }
        });
        ...
        holder.ll_file.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (filesList.get(position).isFile.equalsIgnoreCase("N")) {
                    Intent i = new Intent(activity, SubFolderActivityShared.class);
                    i.putExtra("file_id", movie.fileId);
                    i.putExtra("parent_id", movie.parentId);
                    i.putExtra("file_move_or_not", SubFolderActivityShared.file_move_or_not);
                    ...
                    } else if (file_extension.equalsIgnoreCase(".jpeg")) {
                        openImage(filesList.get(position).filePath, file);
        ...
        holder.profile_image.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (((LinearLayout) activity.findViewById(R.id.header_message_grey)).getVisibility() == View.VISIBLE) {
                    ...
                    fileId = filesList.get(position).fileId;
                    checklistfile.add(new ModelMutilpleDelete(fileId, position));
                    for (int i = 0; i < checklistfile.size(); i++) {
                        Log.e("list", "" + checklistfile.get(i));
                    }
                    ...
                    ln_delete.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            BeanShareDrive movie = filesList.get(position);
                            fileId = movie.fileId;
                            for (int i = 0; i < checklistfile.size(); i++) {
                                file_id = file_id + checklistfile.get(i).getId() + ",";
                            }
                            file_id = file_id.substring(0, file_id.length() - 1);
                            Log.e("fileId", file_id);
                            DeleteMultipleFile(file_id);
        ...
        holder.firstname2.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                fileId = filesList.get(position).fileId;
                if (checklistfile.size() != 0) {
                    for (int i = 0; i < checklistfile.size(); i++) {
                        if (checklistfile.get(i).getId().equalsIgnoreCase(fileId)) {
                            checklistfile.remove(i);
                        }
                    }
                }
    ...
    private void filesharingDialog(int pos, final String fileid, final String oldFileName,
                                   final String isFileFlag, final String file_path, final String icon, final String privecyFlag) {
        ...
        fileshare.setContentView(R.layout.file_shareing_with_me);
        ...
        LinearLayout ll_rename_below_line = (LinearLayout) fileshare.findViewById(R.id.ll_rename_below_line);
        ...
        if (privecyFlag.equalsIgnoreCase("V")) {
            ll_rename.setVisibility(View.GONE);
            ll_rename_below_line.setVisibility(View.GONE);
        } else {
            ll_rename.setVisibility(View.VISIBLE);
            ll_rename_below_line.setVisibility(View.VISIBLE);
        }
        ...
        ll_move_dialog.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
              /*  ShareWithMeFragments.rl_move_or_copy.setVisibility(View.VISIBLE);
                ShareWithMeFragments.upload_float.setVisibility(View.GONE);
                ...
                SubFolderActivity.file_move_or_not = "yes";*/
/*                SubFolderActivity.rl_move_or_copy.setVisibility(View.VISIBLE);
                SubFolderActivity.upload_float.setVisibility(View.GONE);
                AppConstant.MoveFileId = filesList.get(0).fileId;
                AppConstant.MoveFileName = filesList.get(0).folderName;
                AppConstant.MoveFilePath = filesList.get(0).fullPath;
                AppConstant.MoveIsFileFlag = filesList.get(0).isFile;*/
...
// tv_select_person ,tv_visible_person
    ...
    private static void openTagPopuppeople() {
        ...
        adapter1people = new Adapter1FileShareed3(activity2, contact_list);
                            ...
                            if (array.length() != 0) {
                                for (int i = 0; i < array.length(); i++) {
                                    if (array.optJSONObject(i).getString("Status").equalsIgnoreCase("Success")) {
                                        ...
                                        pd.dismiss();
                                        fileshare.dismiss();
                                        ShareWithMeFragments.getFileData();
                                    } else {
                                        pd.dismiss();
                                        Toast.makeText(activity, "" + array.optJSONObject(i).getString("Status_Message"), Toast.LENGTH_SHORT).show();
                                    }
    ...
    private void DeleteMultipleFile(final String fileid) {
        pd.show();
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.File_Delete, new Response.Listener<String>() {
                @Override
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (s != null && s.length() > 0) {
                    ArrayList<BeanShareDrive> temp = new ArrayList<>();
                    for (BeanShareDrive category : array) {
                        if (category.folderName.toLowerCase().contains(s.toLowerCase())) {
                            temp.add(category);
                        }
                    }
                    results.values = temp;
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                filesList = (ArrayList<BeanShareDrive>) filterResults.values;
