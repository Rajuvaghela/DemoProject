...
public class AdapterOrgoEmployee extends RecyclerView.Adapter<AdapterOrgoEmployee.ViewHolder> {
    ...
    public AdapterOrgoEmployee(Activity context, ArrayList<BeanOrgoEmployee> beanOrgoEmployees) {
        this.activity = context;
        this.beanOrgoEmployees = beanOrgoEmployees;
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        Glide.with(activity).load(beanOrgoEmployees.get(position).profileImage).into(holder.proimage);
        holder.tv_name.setText(beanOrgoEmployees.get(position).fullName);
        holder.tv_place.setText(beanOrgoEmployees.get(position).branchName);
        holder.tv_designatin.setText(beanOrgoEmployees.get(position).designationName);

        holder.proimg1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (beanOrgoEmployees.get(position).peopleId != null || beanOrgoEmployees.get(position).peopleId.equalsIgnoreCase("")){
                    Intent intent = new Intent(activity, OtherProfileActivity.class);
                    intent.putExtra("peopleId", beanOrgoEmployees.get(position).peopleId);
        ...
        holder.lnprofile1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                name =  beanOrgoEmployees.get(position).fullName;
                department =  beanOrgoEmployees.get(position).designationName;
                branch =  beanOrgoEmployees.get(position).branchName;
                contactno =  beanOrgoEmployees.get(position).mobileNo;
                emailid =  beanOrgoEmployees.get(position).emailAddress;
