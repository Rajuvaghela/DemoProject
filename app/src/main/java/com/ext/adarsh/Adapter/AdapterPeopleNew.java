...
public class AdapterPeopleNew extends RecyclerView.Adapter<MyViewHolder> implements Filterable {

    private List<BeanPeopleNew> peopleList = new ArrayList<>();
    private List<BeanPeopleNew> array = new ArrayList<>();
    ...
    public AdapterPeopleNew(List<BeanPeopleNew> peopleList, Activity activity, RecyclerView recyclerView) {
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if(viewType == 1){
            View layoutView = LayoutInflater.from(parent.getContext()).inflate(R.layout.people_list_item, parent, false);
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        //    ((ProgressViewHolder)holder).progressBar.setIndeterminate(true);
            ...
            ((MyViewHolder)holder).txtname.setTypeface(Utility.getTypeFaceTab());
            ((MyViewHolder)holder).txtdiscription.setTypeface(Utility.getTypeFace());

            ((MyViewHolder)holder).txtname.setText(peopleList.get(position).fullName);
            ((MyViewHolder)holder).txtdiscription.setText(peopleList.get(position).mobileNo);
            Glide.with(activity).load(peopleList.get(position).profileImage).into(((MyViewHolder)holder).imageview);

            ((MyViewHolder)holder).lnmain.setOnClickListener(new View.OnClickListener() {
                @Override
            ...
            ((MyViewHolder)holder).contactmenu.setOnClickListener(new View.OnClickListener() {
                @Override
    ...
    private void showAlertDialog3(final int pos) {
        ...
        lnviewprofile.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (peopleList.get(pos).peopleId != null || peopleList.get(pos).peopleId.equalsIgnoreCase("")){
    ...
    public void popup(MyViewHolder holder, final int position) {
        ...
        tv6.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                addToFavPeople(peopleList.get(position).peopleId);
            }
        });
    ...
    public void addToFavPeople(final String id) {
        ...
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.People_Add_To_Favorite, new Response.Listener<String>() {
                @Override
    ...
    public void addToContactPeople(final String id,final String groupid) {
        pd.show();
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.People_Add_To_Contact, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Log.e(AppConstant.TAG, response);
                    try {
                        JSONObject object = new JSONObject(response);
                        if (object.has("People_Add_To_Contact")) {
                            JSONArray array = object.getJSONArray("People_Add_To_Contact");
                            if (array.length() != 0) {
                                for (int i = 0; i < array.length(); i++) {
                                    if (array.optJSONObject(i).getString("Status").equalsIgnoreCase("Success")) {
                                        Toast.makeText(activity, "" + array.optJSONObject(i).getString("Status_Message"), Toast.LENGTH_SHORT).show();
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (s != null && s.length() > 0) {
                    ArrayList<BeanPeopleNew> temp = new ArrayList<>();
                    for (BeanPeopleNew category : array) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                peopleList = (ArrayList<BeanPeopleNew>) filterResults.values;
    ...
    public void showpopupmenu(View view,final int position){
        ...
        popup.getMenuInflater()
                .inflate(R.menu.popupmenu_people, popup.getMenu());
        ...
        btn_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (spinner_group.getSelectedItemPosition() == 0){
                    ...
                    addToContactPeople(peopleList.get(position).peopleId,beanGroups.get(spinner_group.getSelectedItemPosition()).categoryId);
                }
            }
        });
    ...
    void getPeopleDetail(final int position,final String id) {
        pd.show();
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.People_Profile, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Log.e(AppConstant.TAG, response);
                    try {
                        ...
                        if (object.has("People_Profile_Array")) {
                            JSONArray jsonArray = object.getJSONArray("People_Profile_Array");
                            if (jsonArray.length() != 0) {
