...
public class AdapterMyDrive extends RecyclerView.Adapter<AdapterMyDrive.MyViewHolder> implements Filterable{

    private List<BeanFiles> filesList;
    private List<BeanFiles> array;
    ...
    static Adapter1FileShare1 adapter1people;
    ...
    public AdapterMyDrive(List<BeanFiles> messqageList, Activity activity) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        final BeanFiles movie = filesList.get(position);
        fileId = movie.fileId;

        holder.title.setText(movie.folderName);
        Log.e("icon", "" + movie.icon);
        Glide.with(activity).load(movie.icon).error(R.drawable.app_logo_icon).into(holder.folderimage);
    ...
    /*            AppConstant.MoveFileId = filesList.get(0).fileId;
                ...
                filesharingDialog(position, movie.fileId, movie.folderName, movie.isFile, movie.filePath, movie.icon, movie.fullPath);
            }
        });
        ...
        holder.ll_file.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (filesList.get(position).isFile.equalsIgnoreCase("N")) {
                    Intent i = new Intent(activity, SubFolderActivity.class);
                    i.putExtra("file_id", movie.fileId);
                    i.putExtra("parent_id", movie.parentId);
                    i.putExtra("file_move_or_not", SubFolderActivity.file_move_or_not);
            ...
            public void onClick(View v) {
                if (((LinearLayout) activity.findViewById(R.id.header_message_grey)).getVisibility() == View.VISIBLE) {
                    ...
                    ln_delete.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            BeanFiles movie = filesList.get(position);
                            fileId = movie.fileId;
                            for (int i=0;i<checklistfile.size();i++){
    ...
    private void filesharingDialog(int pos, final String fileid, final String oldFileName,
                                   final String isFileFlag, final String file_path, final String icon, final String fullPath) {
        ...
        ll_download.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (isFileFlag.equalsIgnoreCase("N")) {
        ...
        ll_move_dialog.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                MyDrive.rl_move_or_copy.setVisibility(View.VISIBLE);
                MyDrive.upload_float.setVisibility(View.GONE);
...
/*                SubFolderActivity.rl_move_or_copy.setVisibility(View.VISIBLE);
    ...
    private void DownloadFolder(final String fileid) {
        pd.show();
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.File_Download, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    ...
                    try {
                        ...
                        if (object.has("File_Download")) {
                            JSONArray array = object.getJSONArray("File_Download");
                            if (array.length() != 0) {
                                for (int i = 0; i < array.length(); i++) {
                                    if (array.optJSONObject(i).getString("Status").equalsIgnoreCase("Success")) {
                                        DownloadFile(array.optJSONObject(i).getString("FilePath"));
                                        fileshare.dismiss();
                                        // fileshare.dismiss();
...
// tv_select_person ,tv_visible_person
    ...
    private static void openTagPopuppeople() {
        ...
        adapter1people = new Adapter1FileShare1(activity2, contact_list);
    ...
    private void DeleteMutipleFile(final String fileid) {
        pd.show();
        if (checkConnectivity()) {
            StringRequest request = new StringRequest(Request.Method.POST, AppConstant.File_Delete, new Response.Listener<String>() {
                @Override
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (s != null && s.length() > 0) {
                    ArrayList<BeanFiles> temp = new ArrayList<>();
                    for (BeanFiles category : array) {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                filesList = (ArrayList<BeanFiles>) filterResults.values;
